...
public abstract class Entity
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            boolean sneakingOnGround = this.onGround && this.isSneaking() && this instanceof EntityPlayer;

            if (sneakingOnGround)
            {
                double var21;

                for (var21 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(x, -1.0D, 0.0D)).isEmpty(); oldX = x)
                {
                    if (x < var21 && x >= -var21)
                    {
                        x = 0.0D;
                ...
                for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.getOffsetBoundingBox(0.0D, -1.0D, z)).isEmpty(); oldZ = z)
                {
                    if (z < var21 && z >= -var21)
                    {
                        z = 0.0D;
            ...
            boolean var37 = this.onGround || oldY != y && oldY < 0.0D;
            int var23;
            ...
            if (this.stepHeight > 0.0F && var37 && (sneakingOnGround || this.ySize < 0.05F) && (oldX != x || oldZ != z))
            {
                var38 = x;
                var25 = y;
                var27 = z;
                ...
                var36 = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.addCoord(oldX, y, oldZ));
            ...
            this.posY = this.boundingBox.minY + (double)this.yOffset - (double)this.ySize;
            this.posZ = (this.boundingBox.minZ + this.boundingBox.maxZ) / 2.0D;
            this.isCollidedHorizontally = oldX != x || oldZ != z;
            this.isCollidedVertically = oldY != y;
            this.onGround = oldY != y && oldY < 0.0D;
            this.isCollided = this.isCollidedHorizontally || this.isCollidedVertically;
            this.updateFallState(y, this.onGround);

            int blockX = MathHelper.floor_double(this.posX);
            int blockY = MathHelper.floor_double(this.posY - 0.20000000298023224D - (double)this.yOffset);
            int blockZ = MathHelper.floor_double(this.posZ);
            Block block = this.worldObj.getBlock(blockX, blockY, blockZ);
            int var33 = this.worldObj.getBlock(blockX, blockY - 1, blockZ).getRenderType();
            ...
            if (oldY != y)
            {
                block.onEntityWalking(this.worldObj, blockX, blockY, blockZ, this);
            }
            ...
            if (this.canTriggerWalking() && !sneakingOnGround && this.ridingEntity == null)
            {


                if (block != Blocks.ladder && block != Blocks.iron_ladder)
                {
                    var25 = 0.0D;
                }
                ...
                if (this.distanceWalkedOnStepModified > (float)this.nextStepDistance && block.getMaterial() != Material.air)
                {
                    ...
                    this.func_145780_a(blockX, blockY, blockZ, block);
                }
            }
    ...
    /**
     * Returns true if the entity is on fire. Used by render to addSmelting the fire effect on rendering.
     */
